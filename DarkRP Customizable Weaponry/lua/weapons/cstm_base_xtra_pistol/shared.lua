...
local CT, clip, mul, ply, wep, sequence, seqdur, activity, bone, matrix, m, AimVec, AvailableReload, AmountToReload, DTInt3, v, model, sprite, pos, ang, drawpos, td, trace, dir, lt, fw, aim, CamData, old, bullet, eyeang, muz, dtint3, par, fov, ScopeStatus, TimeRel, TimeIdle, sp, can, wc, cyc
...
if CLIENT then
	...
	local mat, size, dist, scale, EA, delta, FT, mtr, n, f, wep
    ...
    function SWEP:ViewModelDrawn()
		...
		if self.VElements then
			...
			if entry and entry.color.a == 255 and IsValid(entry.modelEnt) then
				...
				if dtint3 == 1 then
					dir = (aim + Angle(self.AngleDelta.p / -3, self.AngleDelta.y / 3, 0) + self.Owner:GetPunchAngle()):Forward()
				else
					dir = fw
				end
...
local damagemod1, damagemod2, Dir, ent, dot
...
function SWEP:SecondaryAttack()
	...
	if dtint3 == 2 or dtint3 == 18 or dtint3 == 19 or not self.Owner:OnGround() or self.IsReloading == true or self.Owner:KeyDown(IN_SPEED) or self.ShouldBolt == true then
		return
	end
	...
	if (dtint3 == 0 or dtint3 == 3 or dtint3 == 7) and self.ScopeStatus != 1 then
