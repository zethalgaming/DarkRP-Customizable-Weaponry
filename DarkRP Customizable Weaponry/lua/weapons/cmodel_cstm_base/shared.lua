...
if CLIENT then
	local vm, v, model, sprite, pos, ang, drawpos, lt, fw, aim, dir, trace, ply, old, ang, ang2, CamData, m, name, params, tocheck, dist, entry, dir, mdl, mat, size, dt3, dist, scale, EA, delta, FT, mtr, wep, x, y, y2
	...
	function SWEP:PreDrawViewModel(vm, wep)
		if self:GetDTInt(3) == 1 then
			...
			self.Owner:GetViewModel():SetRenderMode(RENDERGROUP_BOTH)
		end
	end
	...
	function SWEP:PostDrawViewModel(vm, wep)
		if self:GetDTInt(3) == 1 then
			...
			if self.DrawTable then
				...
				render.DrawSprite(EyePos() + self.DrawTable.dir * self.DrawTable.dist,  self.DrawTable.size,  self.DrawTable.size, Color(255, 255, 255, 255))
			end
